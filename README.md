...
for species in $(cat species.txt); do
    ...
    if [ ! -f chains/hg38.${species}.all.chain.gz ]; then
        Species=${species^}
...
*Step 6:* For each reference-query pair (where the query species are specified in species.txt), find an orthologous chain for each reference gene in file ref_genes/transcriptCoding.hg38Ens86.clean.bed. This step can take several hours (5-10 hrs per reference-query pair) but it is possible to trivially parallelize the following for loop on multiple cores for a large number of query species. 
```
for species in $(cat species.txt); do
    echo "Finding orthologous chain for each reference gene in ${species} alignments ..."
    if [ ! -f chain_ids/${species}.p ]; then
        python pick_ortho_chains.py -reference hg38 -query ${species} -genes ref_genes/transcriptCoding.hg38Ens86.clean.bed  -geneTranscriptIds ref_genes/hg38_clean.all.genes_id -geneCanonicalTranscriptIds ref_genes/hg38_clean.canonical.genes_id
    fi
done
...
for species in $(cat species.txt); do
    ...
    if [ ! -f chains/hg38.${species}.all.chain.bb ]; then
        ...
        zcat chains/hg38.${species}.all.chain.gz | grep chain | awk '{print $3"\t"$6"\t"$7"\t"$13"\t0\t"$5}' | sort -k1,1 -k2,2n > ${tmpfile}
...
for species in $(cat species.txt); do
    ...
    python extract_coding_seqs.py -reference hg38 -genes ref_genes/transcriptCoding.hg38Ens86.clean.bed -queries ${species} -outdir mappings/
done    
```
...
```
python generate_md_values.py -reference hg38  -speciesList species.txt -geneTranscriptIds ref_genes/hg38_clean.all.genes_id 
```
...
```
python phylo_filter.py -reference hg38 -speciesList species.txt -phyloTree species-tree.nh -geneTranscriptIds ref_genes/hg38_clean.all.genes_id  -outFile hcoErosions.csv
```
